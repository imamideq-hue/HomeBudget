...
export function spentForGroup(
  ...
  for (const t of transactions) {
    ...
    if (getGroupForSub(t.subCategoryId).id !== groupId) continue;
    total += t.amount;
  }
  return total;
}
...
export function calculateGroupBudget(
  ...
  return summarize(group, spentForGroup(group.id, transactions, ref));
}
...
export function calculateGroupBudgets(
  ...
  for (const t of transactions) {
    ...
    spentByGroup.set(group.id, (spentByGroup.get(group.id) ?? 0) + t.amount);
  }
